...
void main() {
  ...
  float height = snoise(vec2(uv.x * 2.0 + uTime * 0.1, uTime * 0.25)) * 0.75 * uAmplitude;
  height = exp(height);
    ...
    <div className="relative flex w-full items-center justify-center bg-black z-0">
      <div
        className={cn(
          ...
          "[background-image:linear-gradient(to_right,#8a2742_1px,transparent_1px),linear-gradient(to_bottom,#8a2742_1px,transparent_1px)]",
        )}
      />
      ...
      <div ref={ctnDom} className="w-full h-40 bg-black z-0" />
    </div>
  );
}
